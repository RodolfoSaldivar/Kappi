...
<?php $this->assign('title', 'Agregar Salón - Kappi');  ?>

<?php $this->set('titulo_vista', 'Agregar Salón'); ?>
<?php $this->set("breadcrumbs",
	'<a href="/ciclos" class="breadcrumb">Ciclos</a>
	<a href="/salones_index/'.$this->request->data["Salone"]["ciclo_id"].'" class="breadcrumb">'.$ciclo["Ciclo"]["nombre"].'</a>
	<a class="breadcrumb">Agregar Salón</a>'
) ?>
...
<?php echo $this->Form->create('Salone'); ?>
	...
	<?php echo $this->Form->hidden('ciclo_id'); ?>
			...
			<div class="row">
				...
				<div class="col s6 padding_top padding_bottom">
					...
					<div class="input-field">
						...
						<input name="data[Salone][nombre]" type="text" id="SaloneNombre" aria-required="true" class="error" aria-invalid="true">
					</div>
					...
					<div class="input-field">
						...
						<select id="SaloneUserId" name="data[Salone][user_id]">
							...
							<option value="nada" disabled selected>Maestro</option>

							<?php foreach ($maestros as $key => $maestro): ?>

								<option value="<?php echo $maestro[0]["encriptada"] ?>"><?php echo $maestro[0]["a_paterno"]." ".$maestro[0]["a_materno"].", ".$maestro[0]["nombre"] ?></option>
				...
				<div class="col s6 padding_top padding_bottom">
					...
					<input type="hidden" id="SaloneHidden" name="data[Salone][grado_id]">

					<div class="input-field">
						<i class="material-icons prefix">book</i>
						<select id="SaloneGradoId">
						...
						<label id="SaloneGradoId-error" class="validation_label" style="position:absolute!important;">*Requerido</label>
	...
	<div class="bg_blanco">
		<div class="contenedor">
			...
			<div id="moduloAgregarSalon" class="row margin_nada padding_bottom">
				...
				<div class="col s6 right" id="segundaMitad" style="display:none">
...
<?php $this->Html->scriptStart(array('inline' => false)); ?>
	...
	$('#SaloneAgregarForm').submit(function(event)
	{
		if(!$('#SaloneUserId').val())
		{
			$("#SaloneUserId-error").css("display", "initial");
			event.preventDefault();
		}

		if(!$('#SaloneGradoId').val())
		{
			$("#SaloneGradoId-error").css("display", "initial");
			event.preventDefault();
		}
	});

	$(document).on("change", "#SaloneUserId", function()
	{
		$("#SaloneUserId-error").css("display", "none");
	});

	$(document).on("change", "#SaloneGradoId", function()
	{
		$("#SaloneHidden").val($('#SaloneGradoId').val());
		$("#SaloneGradoId-error").css("display", "none");
		segundoSelect();
		$('#SaloneGradoId').attr("disabled", true);
		$('#SaloneGradoId').material_select();
	});
	...
	{
		$('#segundaMitad').css("display", "block");
		...
		$.ajax({
	        ...
	        data: {
	        	...
	        	padre: $('#SaloneGradoId').val(),
	        	modo: $('#modo').val(),
	        	ciclo: "<?php echo $this->request->data["Salone"]["ciclo_id"] ?>",
	        	accion: "a"
	        }
	    });
	}
